...
class GuideHandler(GuideStepGetHandler):
    ...
    def get(self):
        ...
        return self.render_to_string("admin/apply/resource/index.html", pro_list_res=pro_list_res, STATUS_RESOURCE=STATUS_RESOURCE)


#@url("/apply/pro_(?P<pro_id>\d+)/resource/add", name="apply.resource.add", active="apply.resource.add")
@url("/apply/resource/add", name="apply.resource.add", active="apply.resource.add")
class GuideHandler(GuideStepGetHandler):
    u'权限申请'
    @check_perms('pro_info.view')
    @unblock
